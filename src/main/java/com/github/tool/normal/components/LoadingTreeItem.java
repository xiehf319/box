...
public class LoadingTreeItem<T> extends TreeItem<T> {
    ...
    public static <T> EventType<TreeModificationEvent<T>> preAddLoadedChildrenEvent() {
        return (EventType<TreeModificationEvent<T>>) PRE_ADD_LOADED_CHILDREN;
    }

    @SuppressWarnings("unchecked")
    public static <T> EventType<TreeModificationEvent<T>> postAddLoadedChildrenEvent() {
